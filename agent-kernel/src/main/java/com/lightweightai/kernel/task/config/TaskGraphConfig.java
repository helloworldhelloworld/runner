 ...
 * tasks:
 *   - name: classify
 *     ref: intent.classifier        # TaskRegistry 中的任务名
 *   - name: retrieve
 *     ref: rag.retrieval
 *     needs: [classify]
 *     join: ALL_SUCCESS
 ...
 *     ref: safety.filter
 *     needs: [retrieve]
 *     guard: status==SUCCESS         # 简单 DSL — 见 GuardExpression
 * expose-as-tools:
 *   - name: my_pipeline
 *     description: ...
 * </pre>
...
public final class TaskGraphConfig {
    ...
    @JsonProperty("expose-as-tools")
    public List<ExposedTool> exposeAsTools = List.of();
    ...
    public static final class ExposedTool {
        @JsonProperty("name")
        public String name;
